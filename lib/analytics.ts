...
/** Keadaan normal pada instalasi baru: schema siap, tetapi transaksi belum diimpor. */
export class AnalyticsSetupRequiredError extends Error {
  readonly code = "ANALYTICS_SETUP_REQUIRED";

  constructor() {
    super(
      "Database sudah terhubung, tetapi belum ada transaksi. Login admin lalu import file Excel/CSV untuk mulai menampilkan dashboard.",
    );
    this.name = "AnalyticsSetupRequiredError";
  }
}
...
function canonicalCategory(category: string) {
  const normalized = category
    ...
    .toUpperCase()
    .replace(/\s*\+\s*/g, " & ");
  if (normalized.includes("ACC") && normalized.includes("IOT"))
    return "ACC & IOT";
...
function makeMetric(
  ...
  return {
    ...
    gap: Math.round(gap),
    targetByDay:
      target > 0 && remaining > 0 ? Math.round(gap / remaining) : null,
...
function raceTarget(
  ...
  return (
    config?.racingTargets?.[sales]?.[key]?.[
      unit === "qty" ? "quantity" : "amount"
    ] || 0
  );
}
...
export async function getAnalytics(
  ...
  if (!optionRows.length) {
    throw new AnalyticsSetupRequiredError();
  }

  const storeMap = new Map(
    masterStores.map((row) => [row.kode, { code: row.kode, name: row.nama }]),
  );
  for (const row of optionRows) {
    if (!storeMap.has(row.siteCode))
      storeMap.set(row.siteCode, { code: row.siteCode, name: row.siteDesc });
  }
  ...
  const periods = Array.from(
    new Map(
      optionRows.map((row) => {
        const value = {
          month: row.orderDate.getUTCMonth() + 1,
          year: row.orderDate.getUTCFullYear(),
        };
  ...
  const storeCode = stores.some((store) => store.code === requestedStore)
    ? requestedStore
    : latestReport &&
        stores.some((store) => store.code === latestReport.storeCode)
  ...
  const previousRows = await prisma.salesTransaction.findMany({
    where: {
      siteCode: storeCode,
      orderDate: periodRange(previous.month, previous.year),
    },
  ...
  const allCategoryMtd = sumFacts(facts, (fact) =>
    CATEGORIES.some(
      (category) => (category.source || category.key) === fact.category,
    ),
  ).mtd;
  const categoryRows = CATEGORIES.map((category) => {
    ...
    const target = people.reduce(
      (sum, sales) => sum + targetFor(config, "categoryTargets", sales, source),
      0,
    );
    return {
      key: source,
  ...
  const peoplePerformance = people.map((name) => {
    ...
    const personMtd = sumFacts(personFacts, (fact) =>
      CATEGORIES.some(
        (category) => (category.source || category.key) === fact.category,
      ),
    );
    const categories = CATEGORIES.map((category) => {
      ...
      return {
        ...
        label: category.label,
        metric: makeMetric(
          targetFor(config, "categoryTargets", name, source),
          value.mtd,
          value.quantity,
          elapsedDays,
          totalDays,
    ...
    return {
      ...
      categories,
      total,
      previousActual: previousActual || null,
      growth: previousActual
        ? ((total.expect - previousActual) / previousActual) * 100
        : null,
    };
  });
  ...
  const performanceTotal = makeMetric(
    totalTarget,
    allCategoryMtd,
    categoryRows.reduce((sum, row) => sum + row.metric.quantity, 0),
    elapsedDays,
    totalDays,
    allCategoryMtd,
  );
  const previousActual =
    previousRows.reduce(
      (sum, row) => sum + Number(row.totalNettAmountExcTax),
      0,
    ) || null;
  ...
  const brandNames = Array.from(
    new Set([
      ...
      ...Object.values(config?.brandTargets || {}).flatMap((targets) =>
        Object.keys(targets),
      ),
    ]),
  )
  ...
  const brands = brandNames
    .map((brand) => {
      ...
      const target = people.reduce(
        (sum, sales) => sum + targetFor(config, "brandTargets", sales, brand),
        0,
      );
      return {
        brand,
        ...
        people: people.map((name) => {
          const personValue = sumFacts(
            deviceFacts,
            (fact) => fact.salesName === name && fact.brandName === brand,
          );
          return {
            name,
            metric: makeMetric(
              targetFor(config, "brandTargets", name, brand),
              personValue.mtd,
              personValue.quantity,
              elapsedDays,
              totalDays,
              value.mtd,
            ),
  ...
  const operatorNames = Array.from(
    new Set([
      ...
      ...Object.values(config?.operatorTargets || {}).flatMap((targets) =>
        Object.keys(targets),
      ),
    ]),
  ).filter(Boolean);
  ...
  const operators = operatorNames.map((name) => {
    ...
    const target = people.reduce(
      (sum, sales) => sum + targetFor(config, "operatorTargets", sales, name),
      0,
    );
    return {
      operator: name,
      ...
      people: people.map((sales) => {
        const personValue = sumFacts(
          carrierFacts,
          (fact) => fact.salesName === sales && operator(fact) === name,
        );
        return {
          name: sales,
          metric: makeMetric(
            targetFor(config, "operatorTargets", sales, name),
            personValue.mtd,
            personValue.quantity,
            elapsedDays,
            totalDays,
  ...
  const racing = raceDefinitions.map((definition) => {
    ...
    return {
      ...
      metric: makeMetric(
        target,
        actual,
        definition.unit === "qty" ? actual : value.quantity,
        elapsedDays,
        totalDays,
      ),
      quantityMetric:
        definition.unit === "amount" && hasQuantityTarget
          ? makeMetric(
              people.reduce(
                (sum, sales) =>
                  sum + raceTarget(config, sales, definition.key, "qty"),
                0,
              ),
              value.quantity,
              value.quantity,
      ...
      people: people.map((name) => {
        ...
        return {
          name,
          metric: makeMetric(
            raceTarget(config, name, definition.key, definition.unit),
            personActual,
            person.quantity,
            elapsedDays,
            totalDays,
  ...
  const focusPeople = people.map((name) => {
    ...
    const repair = sumFacts(
      personFacts,
      (fact) => fact.category === "REPAIR CONTRACT",
    );
    const accessory = sumFacts(
      personFacts,
      (fact) => fact.category === "ACC & IOT",
    );
    const personCarrier = personFacts.filter(
      (fact) => fact.category === "CARRIER",
    );
    const indosat = sumFacts(
      personCarrier,
      (fact) => operator(fact) === "INDOSAT",
    ).quantity;
    const telkomsel = sumFacts(
      personCarrier,
      (fact) => operator(fact) === "TELKOMSEL",
    ).quantity;
    const xl = sumFacts(
      personCarrier,
      (fact) => operator(fact) === "XL PRIO",
    ).quantity;
    const medpoint = sumGrossFacts(
      personFacts,
      (fact) => fact.brandName === "MEDPOIN",
    );
    const tradeIn = sumGrossFacts(personFacts, (fact) =>
      /TRADE\s*IN/.test(fact.articleDescription),
    );
    const tmicro = sumFacts(personFacts, (fact) =>
      /TMICRO|T-MICRO|TELKOMSEL MICRO/.test(fact.articleDescription),
    ).quantity;
    const vidio = sumFacts(personFacts, (fact) =>
      /VIDIO/.test(fact.articleDescription),
    ).quantity;
    ...
    return {
      ...
      repair: {
        ...
        rate: device.quantity ? (repair.quantity / device.quantity) * 100 : 0,
      },
      carrier: {
        indosat,
        telkomsel,
        xl,
        total: carrierTotal,
        rate: device.quantity ? (carrierTotal / device.quantity) * 100 : 0,
      },
      accessory: {
        quantity: accessory.quantity,
        mtd: Math.round(accessory.mtd),
        rate: device.quantity
          ? (accessory.quantity / device.quantity) * 100
          : 0,
  ...
  const totals = {
    ...
    accessoryQty: focusPeople.reduce(
      (sum, row) => sum + row.accessory.quantity,
      0,
    ),
  ...
  return {
    ...
    performance: {
      ...
      previousActual,
      growth: previousActual
        ? ((performanceTotal.expect - previousActual) / previousActual) * 100
        : null,
    },
    brands,
    operators,
